...
public class AlloySmelter extends BlockWithEntity {
    public static final MapCodec<AlloySmelter> CODEC = createCodec(AlloySmelter::new);
    ...
    protected void onStateReplaced(BlockState state, World world, BlockPos pos, BlockState newState, boolean moved) {
        if (state.getBlock() != newState.getBlock()) {
            if (world.getBlockEntity(pos) instanceof AlloySmelterBlockEntity alloySmelterBlockEntity) {
                ItemScatterer.spawn(world, pos, alloySmelterBlockEntity);
    ...
    protected ItemActionResult onUseWithItem(ItemStack stack, BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        if (!world.isClient) {
            NamedScreenHandlerFactory screenHandlerFactory = ((AlloySmelterBlockEntity) world.getBlockEntity(pos));
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return validateTicker(type, ModBlockEntities.ALLOY_SMELTER_BLOCK_ENTITY, (world1, pos, state1, blockEntity) -> blockEntity.tick(world1, pos, state1));
    }
    ...
    public void randomDisplayTick(BlockState state, World world, BlockPos pos, Random random) {
        ...
        if (world.getBlockEntity(pos) instanceof AlloySmelterBlockEntity alloySmelterBlockEntity && !alloySmelterBlockEntity.getStack(1).isEmpty()) {
            world.addParticle(new ItemStackParticleEffect(ParticleTypes.ITEM, alloySmelterBlockEntity.getStack(1)),
